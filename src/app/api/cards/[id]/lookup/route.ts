...
export async function GET(
  ...
  try {
    const code = normalizeCardCode(params.id)
    ...
    if (error || !card) {
      await auditLookup({ headers: req.headers, cardId: null, outcome: 'not_found', surface: 'cards-lookup' })
      return NextResponse.json({ error: 'Card not found' }, { status: 404 })
    }

    await auditLookup({ headers: req.headers, cardId: card.id, outcome: 'found', surface: 'cards-lookup' })
